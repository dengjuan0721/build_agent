...
from base_model_markdown.json_cache import JsonFileStateCache
...
class MCP_ChatBot:
    ...
    async def process_query(self, query: str, session_id: str = "default_session"):
        # --- 步骤 0: 初始化当前会话的状态缓存 ---
        # 【核心改动】将 RedisStateCache 替换为 JsonFileStateCache
        state_cache = JsonFileStateCache(session_id)

        # --- 特殊命令：清空缓存 ---
        if query.strip().lower() == "clear cache":
            state_cache.clear()
            print(f"Cache for session '{session_id}' has been cleared.")
            return
        ...
        while True:
            ...
            if response_message.tool_calls:
                ...
                for tool_call in response_message.tool_calls:
                    ...
                    # --- 【新的缓存逻辑】 ---
                    # 1. 检查缓存
                    cached_result = state_cache.get(function_name, function_args)

                    if cached_result:
                        # 2. 缓存命中！
                        print(f"CACHE HIT for '{function_name}'. Reusing cached result.")
                        tool_output_string = cached_result
                    else:
